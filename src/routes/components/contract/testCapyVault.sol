...
contract CAPYVault is ERC4626, Ownable, ReentrancyGuard {
    ...
    uint256 private constant DAY_IN_SECONDS = 10;
   
    ERC20 public capyToken;

    uint256 private constant BASIS_POINT_SCALE = 1e4;
    uint256 private entryFeeBps;
    uint256 private exitFeeBps;
    uint256 private devFeeBps;
    uint256 public constant WITHDRAWAL_INTERVAL = 60 seconds;
    ...
    constructor(ERC20 _asset, string memory _name, string memory _symbol, address _devAddress, uint256 _initialETHDeposit)
        ERC4626(_asset)
        ERC20(_name, _symbol)
        Ownable(msg.sender)
        ReentrancyGuard()
        payable       
    {
        ...
        entryFeeBps = 400; // 4% in basis points
        exitFeeBps = 500; // 5% in basis points
        devFeeBps = 100; // 1% in basis points
        devAddress = _devAddress;

        if (_initialETHDeposit > 0) {
            uint256 totalInitialShares = 10;
            uint256 ownerShares = totalInitialShares * 10 / 100;

            _mint(owner(), ownerShares);
            _mint(address(this), totalInitialShares - ownerShares);
        }
    }
    ...
    function calculateFee(uint256 assets, uint256 feeBps, Rounding rounding) private pure returns (uint256) {
        uint256 fee = (assets * feeBps) / BASIS_POINT_SCALE;
        if (rounding == Rounding.Up && (assets * feeBps % BASIS_POINT_SCALE) > 0) {
            fee += 1; 
        }
        return fee;
    }
    ...
    function deposit(uint256 _assets) public nonReentrant {
        ...
        uint256 fee = _feeOnTotal(_assets, entryFeeBps);
    ...
    function withdraw(uint256 _shares, address _receiver) public nonReentrant {
        ...
        uint256 devFee = _feeOnRaw(assets, devFeeBps); 
        uint256 fee = _feeOnRaw(assets, exitFeeBps) + devFee;
    ...
    function _feeOnRaw(uint256 assets, uint256 feeBps) private pure returns (uint256) {
        return assets * feeBps / BASIS_POINT_SCALE;
    }

    function _feeOnTotal(uint256 assets, uint256 feeBps) private pure returns (uint256) {
        return assets * feeBps / (feeBps + BASIS_POINT_SCALE);
    }
    
    function setCapyTokenAddress(ERC20 _capyToken) public onlyOwner {
